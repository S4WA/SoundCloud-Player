...
function init(){chrome.storage.sync.get(null,(items)=>{json=items});chrome.tabs.query({url:"*://soundcloud.com/*"},(results)=>{if(results.length==0){ready=!1;json.artwork="";json.track="* Click to Open SoundCloud *";chrome.storage.sync.set(json,null)}});$("#version").text("v"+chrome.runtime.getManifest().version);registerElements();$(toggleElem).on("click",()=>{toggle()});$(prevElem).on("click",()=>{queue("prev")});$(nextElem).on("click",()=>{queue("next")});$(favElem).on("click",()=>{toggleFav()});$(trackElem).on("click",()=>{openSCTab()});$(artworkElem).on("click",()=>{openSCTab()});$(repeatElem).on("click",()=>{repeat()});$(shuffleElem).on("click",()=>{queue("shuffle")})}
