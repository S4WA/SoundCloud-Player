document.addEventListener("DOMContentLoaded",()=>{init();ready=!0});chrome.runtime.onMessage.addListener(function(message,sender,sendResponse){if(message.type!="update"||message.value==null)return;var items=message.value;if(!ready)return;if((items.artwork!=null&&items.artwork!="")&&items.artwork!=artworkElem.src){artworkElem.src=items.artwork}
...
json=items});function init(){chrome.tabs.query({url:"*://soundcloud.com/*"},(results)=>{if(results.length==0){ready=!1;json={};json.artwork="";json.track="* Click to Open SoundCloud *";json.time=null;json.volume=0;$("#close").remove();$("#second br:last-child").remove();$("#controller").remove();$("hr:first-child").remove()}});$("#version").text("v"+chrome.runtime.getManifest().version);registerElements();registerEvents()}
...
function queue(request,value){if(!ready)return;request=request.toLowerCase();chrome.tabs.query({url:"*://soundcloud.com/*"},(results)=>{if(results.length!=0){var jsonRequest={}
jsonRequest.type=request;if(!value)jsonRequest.value=value;chrome.tabs.sendMessage(results[0].id,jsonRequest,null)}})}
...
function repeat(){if(json.repeat==null||repeatElem==null)return;queue("repeat");repeatElem.value="Repeat ("+json.repeat+")"}
