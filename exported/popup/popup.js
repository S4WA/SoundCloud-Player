document.addEventListener("DOMContentLoaded",()=>{init()});window.onload=()=>{setInterval(updater(),500);ready=!0}
function updater(){chrome.storage.sync.get(null,function(items){if(!ready)return;if((items.artwork!=null&&items.artwork!="")&&items.artwork!=artworkElem.src){artworkElem.src=items.artwork}
...
json=items})}
function init(){chrome.storage.sync.get(null,(items)=>{json=items});chrome.tabs.query({url:"*://soundcloud.com/*"},(results)=>{if(results.length==0){ready=!1;json.artwork="";json.track="* Click to Open SoundCloud *";json.time=null;json.volume=0;chrome.storage.sync.set(json,null);$("#close").remove();$("#second br:last-child").remove();$("#controller").remove()}});$("#version").text("v"+chrome.runtime.getManifest().version);registerElements();registerEvents()}
