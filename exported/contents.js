window.onload=()=>{var ready=!1;setInterval(()=>{ready=($(".playControls__wrapper").length!=0);var playing=$(".playControl")[0].title=="Pause current",track=$("a.playbackSoundBadge__titleLink")[0].title+" By "+$("a.playbackSoundBadge__lightLink")[0].title,artwork=$(".playbackSoundBadge span.sc-artwork").css("background-image").replace("url(\"","").replace("\")","").replace("50x50.","500x500."),fav=$(".playControls__soundBadge .sc-button-like")[0].title=="Unlike",current=$(".playbackTimeline__timePassed span[aria-hidden]").text(),end=$(".playbackTimeline__duration span[aria-hidden]").text(),volume=Number($(".volume__sliderWrapper").attr("aria-valuenow"))*100;if(ready&&json.track!=track){json.track=track;json.artwork=artwork;post()}
...
if(json.volume!=volume){json.volume=volume;post()}},500)}
